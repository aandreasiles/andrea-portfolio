...
export const TimelineItem = ({ item }: TimelineItemProps) => (
  <div className="relative pl-8 sm:pl-12 py-6 group">
    ...
    {/* Periodo en timeline solo visible en desktop */}
    <time className="hidden sm:block absolute -left-28 sm:-left-32 top-8 text-sm font-bold text-cyan-400 uppercase tracking-wider">{item.period}</time>
    <div className="p-6 bg-gradient-to-br from-gray-800/50 to-gray-700/50 rounded-2xl border border-gray-700/50 group-hover:border-cyan-400/50 transition-all duration-300 transform group-hover:scale-105">
      {/* Periodo visible solo en móvil dentro de la caja */}
      <time className="block sm:hidden mb-2 text-sm font-bold text-cyan-400 uppercase tracking-wider">{item.period}</time>
